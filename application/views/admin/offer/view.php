              ...
              <div class="col-12">
                <div class="page-title-box">
                  <h4 class="page-title float-left">Offers</h4>
                    ...
                    <button type="button" class="btn btn-gradient btn-rounded waves-light waves-effect w-md" data-toggle="modal" data-target="#add-offer">Add offer</button>
                  </ol>
                    ...
                    <tbody>
                      <?php foreach($offers as $offer) {?>
                       <tr>
                         ...
                         <td><?php echo date('d-M-Y',strtotime($offer->date_from));?></td>
                         <td><?php echo date('d-M-Y',strtotime($offer->date_to));?></td>
                         <td><?php echo date('h:i A',strtotime($offer->time_from));?></td>
                         <td><?php echo date('h:i A',strtotime($offer->time_to));?></td>
                         <td><?=$offer->status?></td>
                         <td><button type="button" class="btn btn-link" style="font-size:20px;color:blue" onclick="edit('<?=$offer->offer_id?>')"><i class="fa fa-pencil"></i></button></td>
                         <?php if($offer->status=='Active') {?>
                            <td>  <a class="btn btn-link" style="font-size:16px;color:red" href="<?=site_url('admin/offer/disable/'.$offer->offer_id)?>"  onclick="return block()">Block</i></a></td>
                         <?php } else {?>
                            <td>  <a class="btn btn-link" style="font-size:16px;color:red" href="<?=site_url('admin/offer/enable/'.$offer->offer_id)?>"  onclick="return block()">Enable</i></a></td>
                         <?php };?>
                       </tr>
                      <?php };?>
                    </tbody>
             ...
             <div class="modal-dialog">
                 <div class="modal-content">
                     ...
                     <div class="modal-body">
                         ...
                         <form action="<?=site_url('admin/offer/addData')?>" method="post" id="add-form" enctype="multipart/form-data">
                             ...
                             <div class="form-group m-b-25">
                                 <div class="col-12">
                                     ...
                                     <input type="date" name="date_from" id="date_from" placeholder="Image URL" class="form-control" onchange="Checkfromdate()" required>
                                 </div>
                             </div>
                             ...
                             <div class="form-group m-b-25">
                                <div class="col-12">
                                    <label for="select">End date</label>
                                     <input type="date" name="date_to" id="date_to" placeholder="Image URL" class="form-control" onchange="CheckTodate()" required>
                                </div>
                             </div>
        ...
        <div id="edit-offer" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
             <div class="modal-dialog">
                 <div class="modal-content">
                     ...
                     <div class="modal-body">
                         <h2 class="text-uppercase text-center m-b-30">
                             <span><h4>Edit Offer</h4></span>
                         </h2>

                         <form action="<?=site_url('admin/offer/Update')?>" method="post" id="add-form" enctype="multipart/form-data">
                            <input type="hidden" name="offer_id" id="offer_id">
                             ...
                             <div class="form-group m-b-25">
                                <div class="col-12">
                                    <label for="select">End date</label>
                                     <input type="date" name="date_to" id="end_date" placeholder="Image URL" class="form-control" onchange="CheckTodate()" required>
                                </div>
                             </div>
   ...
   function edit(id)
    {
      ...
      $.ajax({
          method: "POST",
          url: "<?php echo site_url('admin/offer/getOfferById');?>",
          dataType : "json",
          data : { id : id },
          success : function( data ){
            $('#offer_percentage').val(data.percentage);
            $('#start_date').val(data.date_from);
            $('#end_date').val(data.date_to);
